...
async function loadLinesFromImp(impbatch_key) {
    var connection = await getConn();

    try {
        const [results_lines, fields] = await connection.query(
            'SELECT impline_key, impline_name FROM imp_lines WHERE impbatch_key = ?',
            [impbatch_key]
        );
        ...
        for (const line of results_lines) {
            ...
            //insert line
            try {
                const [results_insertline, fields] = await connection.query(
                    'INSERT INTO lines VALUES (null, ?)',
                    [line.impline_name]
                );
                var line_key = results_insertline.insertId;
            ...
            try {
                ...
                for (const line of results) {
                    ...
                    let res = await fetch('https://www.dpmp.cz' + line.impline_name);
